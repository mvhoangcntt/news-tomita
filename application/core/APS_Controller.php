...
class Admin_Controller extends APS_Controller
{
    ...
    public function __construct()
    {
        ...
        //load helper
        $this->load->helper(array('authorization', 'image', 'link', 'format'));
        //Load config
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            if ($this->ion_auth->in_group(1) != true) {
                ...
                if (!in_array($controller, array('dashboard'))) {
                    if ($controller === 'users' && $method !== 'profile') {
...
class Public_Controller extends APS_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->lang->load(array('auth', 'ion_auth', 'frontend', 'account', 'general', 'profile'));
