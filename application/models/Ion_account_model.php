 ...
 */
class Ion_account_model extends CI_Model
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->tables = $this->config->item('tables', 'ion_account');
        // initialize data
        $this->identity_column = $this->config->item('identity', 'ion_account');
        $this->identity_alt_column = $this->config->item('identity_alt', 'ion_account');
        $this->identity_phone_column = $this->config->item('identity_phone', 'ion_account');
        $this->store_salt = $this->config->item('store_salt', 'ion_account');
        $this->salt_length = $this->config->item('salt_length', 'ion_account');
        $this->join = $this->config->item('join', 'ion_account');

        // initialize hash method options (Bcrypt)
        $this->hash_method = $this->config->item('hash_method', 'ion_account');
        $this->default_rounds = $this->config->item('default_rounds', 'ion_account');
        $this->random_rounds = $this->config->item('random_rounds', 'ion_account');
        $this->min_rounds = $this->config->item('min_rounds', 'ion_account');
        $this->max_rounds = $this->config->item('max_rounds', 'ion_account');

        // initialize messages and error
        $this->messages    = array();
        $this->errors      = array();
        $delimiters_source = $this->config->item('delimiters_source', 'ion_account');
        ...
        else
        {
            ...
            $this->message_start_delimiter = $this->config->item('message_start_delimiter', 'ion_account');
            $this->message_end_delimiter = $this->config->item('message_end_delimiter', 'ion_account');
            $this->error_start_delimiter = $this->config->item('error_start_delimiter', 'ion_account');
            $this->error_end_delimiter = $this->config->item('error_end_delimiter', 'ion_account');
        }
        ...
        if ($this->hash_method == 'bcrypt')
        {
            ...
            $params['salt_prefix'] = $this->config->item('salt_prefix', 'ion_account');
...
//        if(empty($query)) $query=$this->db->where($this->identity_alt_column, $identity)
//            ->limit(1)
//            ->count_all_results($this->tables['users']);
    ...
    public function forgotten_password_complete($code, $salt = FALSE)
    {
        ...
        if ($profile)
        {
            ...
            if ($this->config->item('forgot_password_expiration', 'ion_account') > 0)
            {
                //Make sure it isn't expired
                $expiration = $this->config->item('forgot_password_expiration', 'ion_account');
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array())
    {
        ...
        else if (!$this->config->item('default_group', 'ion_account') && empty($groups))
        {
            $this->set_error('account_creation_missing_default_group');
            return FALSE;
        }
        // check if the default set in config exists in database
        $query = $this->db->get_where($this->tables['groups'], array('name' => $this->config->item('default_group', 'ion_account')), 1)->row();
    ...
    public function login($identity, $password, $remember=FALSE)
    {
        ...
        $this->trigger_events('extra_where');
        $identity_column = filter_var($identity, FILTER_VALIDATE_EMAIL)? $this->identity_column : (is_numeric($identity) ? $this->identity_phone_column : $this->identity_alt_column);
        $query = $this->db->select($this->identity_column .', ' . $this->identity_alt_column.', '. $this->identity_phone_column. ', id, password, active, last_login')
...
//                $this->set_session($user);
    ...
    {
        $recheck = (NULL !== $this->config->item('recheck_timer', 'ion_account')) ? $this->config->item('recheck_timer', 'ion_account') : 0;
        ...
        if ($recheck !== 0)
        {
            ...
            if ($last_login + $recheck < time())
            {
                ...
                else
                {
                    ...
                    $identity = $this->config->item('identity', 'ion_account');
    ...
    {
        if ($this->config->item('track_login_attempts', 'ion_account'))
        {
            $max_attempts = $this->config->item('maximum_login_attempts', 'ion_account');
            if ($max_attempts > 0)
            {
    ...
    public function get_attempts_num($identity, $ip_address = NULL)
    {
        if ($this->config->item('track_login_attempts', 'ion_account'))
        {
            ...
            if ($this->config->item('track_login_ip_address', 'ion_account'))
            {
                if (!isset($ip_address))
                {
                    $ip_address = $this->_prepare_ip($this->input->ip_address());
            ...
            $this->db->where('time >', time() - $this->config->item('lockout_time', 'ion_account'), FALSE);
    ...
    {
        if ($this->config->item('track_login_attempts', 'ion_account') && $this->config->item('track_login_ip_address', 'ion_account'))
        {
    ...
    public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
    {
        if ($this->config->item('track_login_attempts', 'ion_account'))
        {
            ...
            $old_attempts_expire_period = max($old_attempts_expire_period, $this->config->item('lockout_time', 'ion_account'));
    ...
    public function user($id = NULL)
    {
        ...
        $id = isset($id) ? $id : $this->session->userdata['account']['account_id'];
        $this->limit(1);
        ...
        foreach ($group_ids as $group_id)
        {
            ...
            )
            {
                ...
                else
                {
                    ...
                    $group_name = !empty($group[0]->name) ? $group[0]->name : '';
    ...
    public function set_lang($lang = 'en')
    {
        ...
        if($this->config->item('user_expire', 'ion_account') === 0)
        {
        ...
        {
            $expire = $this->config->item('user_expire', 'ion_account');
        }
    ...
    public function remember_user($id)
    {
        ...
        if ($this->db->affected_rows() > -1)
        {
            ...
            set_cookie(array(
                'name'   => $this->config->item('identity_cookie_name', 'ion_account'),
                'value'  => $user->{$this->identity_column},
                'expire' => $expire
            ));

            set_cookie(array(
                'name'   => $this->config->item('remember_cookie_name', 'ion_account'),
                'value'  => $salt,
                'expire' => $expire
            ));
    ...
    public function login_remembered_user()
    {
        ...
        if (!get_cookie($this->config->item('identity_cookie_name', 'ion_account'))
            || !get_cookie($this->config->item('remember_cookie_name', 'ion_account'))
            || !$this->identity_check(get_cookie($this->config->item('identity_cookie_name', 'ion_account'))))
        {
        ...
        $query = $this->db->select($this->identity_column . ', id, email, last_login')
            ->where($this->identity_column, urldecode(get_cookie($this->config->item('identity_cookie_name', 'ion_account'))))
            ->where('remember_code', get_cookie($this->config->item('remember_cookie_name', 'ion_account')))
            ->where('active', 1)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
...
//            $this->set_session($user);
            ...
            if ($this->config->item('user_extend_on_login', 'ion_account'))
            {
                $this->remember_user($user->id);
            }
    ...
    public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
    {
        ...
        if ($this->config->item('admin_group', 'ion_account') === $group->name && $group_name !== $group->name)
        {
    ...
    public function delete_group($group_id = FALSE)
    {
        ...
        if($group->name == $this->config->item('admin_group', 'ion_account'))
        {
