...
class NavsMenu
{
   ...
   * @var string
   */
  private $item_tag_open = '<li class="%s">';
   ...
   * @var string
   */
  private $parent_tag_open = '<li class="%s">';
   ...
   * @var string
   */
  private $parent_anchor = '<a href="%s" title="%s" target="%s">%s</a>';
   ...
   * @var string
   */
  private $item_anchor = '<a class="smooth" href="%s" title="%s" target="%s">%s</a>';
      ...
      if (isset($item[$this->menu_label], $item[$this->menu_key])) {
        ...
        if ($has_children) {
          if (is_null($item[$this->menu_parent]) && $this->parentl1_tag_open != '') {
            $tag_open = sprintf($this->parentl1_tag_open, $icon);
        ...
        $label_title = empty($item['icon']) ? $label : '<em><i class="' . $item['icon'] . '"></i></em>';
        $target = $item['type'] == 'other' ? '_blank' : '';
        if (substr_count($item_anchor, '%s') == 4) {
          $html .= sprintf($item_anchor, $href, $label, $target, $label_title);
        } else {
          $html .= sprintf($item_anchor, '#', $label, $target, $label_title);
        }
