...
function get_curl($url,$post = array()){
	do{
		$status = false;
		$ch=curl_init();
		...
		//curl_setopt($ch, CURLOPT_PROXY, '91.75.24.162:3128');//proxy fake ip truy cap
		...
		// config curl link https
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        // end config curl link https
		...
		if($info['http_code'] == 301){
			...
			$url = str_replace('Location:','',$info[11]);
    ...
    curl_close($ch);
    //return array('content' => $result,'http_code' => $info['http_code']);
    return $result;
}

function post_curl($url, $arr_post) {
        $ch = curl_init();
        $defaults = array(
            CURLOPT_POST => 1,
            CURLOPT_HTTPHEADER, array(
                'Content-Type: application/json',
                'Content-Length: ' . strlen(json_encode($arr_post))),
            CURLOPT_URL => $url,
            CURLOPT_FRESH_CONNECT => 1,
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_FORBID_REUSE => 1,
            CURLOPT_TIMEOUT => 30,
            CURLOPT_POSTFIELDS => $arr_post,
            CURLOPT_USERAGENT => "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:x.x.x) Gecko/20041107 Firefox/x.x",
...
function cleanInput($html) {
	$html = trim(html_entity_decode($html, ENT_NOQUOTES, "UTF-8"));
	$search = array(
	...
	//$output = preg_replace($search, '', trim($input));
	$html = strip_tags($html,'<div><p><img><span><strong><b><u><i><ul><ol><li><table><tr><th><td><br>');
	return $html;
}
