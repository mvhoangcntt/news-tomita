	...
	public function ajax_list()
	{
		if($this->input->server('REQUEST_METHOD') == 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
			...
			if(!empty($list)) foreach ($list as $item) {
				...
				$row[] = $item->title;
				$row[] = '<a class="fancybox" href="'.getImageThumb($item->thumbnail).'"><img src="'.getImageThumb($item->thumbnail,200,50).'" style="background-color:#0c7ede"></a>';
	...
	private function _validate()
	{
		if($this->input->server('REQUEST_METHOD') == 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			if (!empty($this->config->item('cms_language'))) foreach ($this->config->item('cms_language') as $lang_code => $lang_name) {
				...
				$this->form_validation->set_rules('property_id', $this->lang->line('form_position'), 'required');
			}
