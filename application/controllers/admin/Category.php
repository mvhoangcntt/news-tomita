        ...
        if($this->input->server('REQUEST_METHOD') == 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            if (!empty($this->config->item('cms_language'))) foreach ($this->config->item('cms_language') as $lang_code => $lang_name) {
                if ($lang_code === $this->config->item('default_language')) {
                    $this->form_validation->set_rules('title[' . $lang_code . ']', 'Tiêu đề' . ' - ' . $lang_name, 'required|trim|min_length[2]|max_length[300]');
                    $this->form_validation->set_rules('meta_title[' . $lang_code . ']', 'Meta title' . ' - ' . $lang_name, 'required|trim|min_length[2]|max_length[300]');
