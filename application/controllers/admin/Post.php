...
class Post extends Admin_Controller
{
    ...
    public function ajax_edit($id)
    {
        ...
        $data[1]->displayed_time = formatDateb($data['0']->displayed_time);
        $data[1]->expiration_time = formatDateb($data['0']->expiration_time);
        ...
        if($this->input->server('REQUEST_METHOD') == 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            if (!empty($this->config->item('cms_language'))) foreach ($this->config->item('cms_language') as $lang_code => $lang_name) {
                if ($lang_code === $this->config->item('default_language')) {
                    $this->form_validation->set_rules('title[' . $lang_code . ']', 'tiêu đề' . ' - ' . $lang_name, 'required|trim|min_length[5]|max_length[300]');
                    $this->form_validation->set_rules('meta_title[' . $lang_code . ']', 'meta title' . ' - ' . $lang_name, 'required|trim|min_length[5]|max_length[300]');
                    $this->form_validation->set_rules('description[' . $lang_code . ']', 'tóm tắt' . ' - ' . $lang_name, 'required');
                    $this->form_validation->set_rules('meta_description[' . $lang_code . ']', 'meta description' . ' - ' . $lang_name, 'required');
                    $this->form_validation->set_rules('meta_keyword[' . $lang_code . ']', 'meta keyword' . ' - ' . $lang_name, 'required');
                }
            }
